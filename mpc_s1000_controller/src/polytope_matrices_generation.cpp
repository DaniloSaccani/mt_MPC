...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
  {
    ...
    if (num_measurements > 0)
    {
      ...
      ros::param::get("/polytope_generation_param/reduction_dist", red_dist);
      ...
      // Inset the polytope by safe_dist
      // BEGIN OF POLYTOPE INSET
      // v3 <--n2--> v2 <--n1--> v1
      float nx[num_vertices], ny[num_vertices], nL, bx, by, bL, l;
      int j = 0;

      for(int i=0; i<num_vertices; i++) //compute polygon edges normals
      {
        // normal i is computed from vertex i to vertex i+1
        j = (i+1)%num_vertices; //index off successive vertex
        nx[i] = poly.polygon.points[i].y - poly.polygon.points[j].y;
        ny[i] = poly.polygon.points[j].x - poly.polygon.points[i].x;
        // normalize normals
      ...
      for(int i=0; i<num_vertices; i++) //move vertices
      {
        // vertex i is moved based on normal i and normal i-1
        j = (i-1)%num_vertices; //index of previous normal
        j = (num_vertices + ((i-1)%num_vertices)) % num_vertices;
        ...
        l = red_dist / sqrt(1 + (nx[i]*nx[j] + ny[i]*ny[j])) * sqrt(2);    // srqt(2) scale to get right offset
        // copy vertex and move it by l in direction b
        poly.polygon.points[i].x += l*bx;
        poly.polygon.points[i].y += l*by;
      }
      // END OF POLYTOPE INSET
      ...
      for(int i=0; i<num_vertices; i++)
      {
        ...
        for(int k=0; k<num_vertices; k++)
        {
          ...
          if(a1*poly.polygon.points[k].x+ a2*poly.polygon.points[k].y >= b)
          {
            a1 = -a1;
            a2 = -a2;
            b = -b;
