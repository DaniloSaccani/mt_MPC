...
# DA MODIFICARE
filepath="/root/mtMPC_ws/src/mpc_s1000_controller/src/data.mat"
...
Aeq = data['Aeq']
beqwithoutx0 = data['beqwithoutx0']
Aacc = data['Aacc']
Aineq2row = data['Aineq2row']
Aineq3row = data['Aineq3row']
bacc1temp = data['bacc1temp']
bacc2temp = data['bacc2temp']

bineq2rowtemp = data['bineq2rowtemp']
bineq3rowtemp = data['bineq3rowtemp']
f1temp = data['f1temp']
f2temp = data['f2temp']
H = data['H']
...
class mtMPC():
    ...
    def solve_qp(self,Ac,bc):
        ...
        Aineq1row = np.dot(Acbar,Gamma_ys)
        Aineq = np.concatenate((Aineq1row,Aineq2row,Aineq3row,Aineq4row),axis=0)
        ...
        bineq4row = bacc
        bineq = np.concatenate((bineq1row,bineq2row,bineq3row,bineq4row),axis=0)
        ...
        # solving the QP
        sol = solve_qp(H,f.T,Aineq,bineq,Aeq,beq)
        if sol is None:
            print('infeasible')
        else:
            print('feasible')
...
def main():
    ...
    # This is where we calculate the trajectory
        ...
        ticqp = time.time()
        sol = controller.solve_qp(controller.Ac,controller.bc)
        tocqp = time.time()
        # definition of the new setpoint
        if sol is None :
            # print("no solution")
            rospy.logwarn("no solution")
            sol = controller.solve_qp(controller.Ac_old,controller.bc_old)
            if sol is None:
