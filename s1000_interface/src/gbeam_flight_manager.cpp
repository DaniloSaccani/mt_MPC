	...
	while(ros::ok())
	{
		...
		switch(flight_phase)
		{
			...
			case 10://drone airborne, ready to start mission
				...
				if(play_press)
				{
					ROS_INFO("GBEAM starting");
					mapping_status_srv.request.request = true;
					if(ros::service::call("gbeam/set_mapping_status", mapping_status_srv))
					if(mapping_status_srv.response.response)
					{
						ROS_INFO("GBEAM started");
						flight_phase = 20;
					}
				}
				break;
			case 20://drone mapping
				if(play_press)
				{
					ROS_INFO("GBEAM stopping");
					mapping_status_srv.request.request = false;
					if(ros::service::call("gbeam/set_mapping_status", mapping_status_srv))
					if(mapping_status_srv.response.response)
					{
						ROS_INFO("GBEAM stopped");
						flight_phase = 80;
					}
				}
				break;
