...
ros::Publisher filter_state_pub;

float K_pos_xy, K_pos_z, K_yaw, vel_max, yaw_rate_max;
float spd_filter = 0.5;
float filtered_pos_x_prev = 0, filtered_pos_y_prev = 0, filtered_pos_z_prev = 0;
...
int main(int argc, char **argv)
{
	...
	filter_state_pub = n.advertise<geometry_msgs::Point>("/s1000_interface/filter_state",1);
	...
	while(n.ok())
	{
		...
		geometry_msgs::Point filt_state;

		filt_state.x = filtered_pos_x_prev;
		filt_state.y = filtered_pos_y_prev;
		filt_state.z = filtered_pos_z_prev;

		filter_state_pub.publish(filt_state);
		...
		filtered_pos_z_prev = (1-spd_filter) * filtered_pos_z_prev + spd_filter * abs_ref.position.z;


		// compute speed references
		// float x_vel = K_pos_xy * (abs_ref.position.x - pos.x);
		// float y_vel = K_pos_xy * (abs_ref.position.y - pos.y);
		...
		float z_vel = K_pos_xy * (filtered_pos_z_prev - pos.z);
